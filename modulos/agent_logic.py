    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if tool_calls:
                ...
                # Evitamos el error "the JSON object must be str, bytes or bytearray, not dict"
                args = tool_calls[0]["function"]["arguments"]
                
                if isinstance(args, dict):
                    # Si Ollama ya nos devuelve un diccionario de Python, lo usamos directamente
                    decision = args
                else:
                    # Si nos devuelve un texto (string), lo convertimos con json.loads
                    decision = json.loads(args)
            else:
                # CORRECCIÓN: Se ha eliminado la URL que se coló en el nombre de la variable (dhttp://...)
            ...
            # Captura errores de red, de Ollama o de parseo de JSON
